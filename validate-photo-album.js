...
const checks = [
  ...
  ["6枚は右側の点線上へ豆図またはQRを重ねる", /if\(sixPhotoLayout&&\(useMiniMap\|\|\(useMapQr&&miniMapAllowed\)\)\)[\s\S]*const commentArea=[\s\S]*const miniWidth=commentArea\.width\*\.45[\s\S]*makePhotoMiniMapImage\(item,miniGeometry/],
  ["6枚豆図領域を上へ拡張して下端を境界線へ合わせる", /if\(sixPhotoLayout&&\(useMiniMap\|\|\(useMapQr&&miniMapAllowed\)\)\)[\s\S]*verticalExtension=6[\s\S]*miniBottom=slotBottom\*rowPx[\s\S]*height:Math\.max\(1,miniBottom-miniTop\)[\s\S]*\{zoom:1\.25,markerScale:1\.25\}/],
