...
for(const token of [
  'id="registryMapAutoBtn" type="button" aria-pressed="false">国土地調査境界</button>',
...
(async()=>{
  ...
  if(result.initial.active||result.initial.pressed!=="false"||result.initial.text!=="国土地調査境界")throw new Error(`registry initial state failed: ${JSON.stringify(result.initial)}`);
