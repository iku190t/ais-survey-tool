...
(async()=>{
  ...
  if(blankWorkspace.count!==9||blankWorkspace.records!==3||blankWorkspace.beeps!==3)throw new Error(`GPS-only registration failed: ${JSON.stringify(blankWorkspace)}`);
  ...
  console.log("OK: Drogger 3-second hold, minimized accuracy, GPS-only registration, beep, increment, metadata and grouped undo");
