...
(async()=>{
  ...
  for(let i=0;i<5;i++)await page.locator("#gpsTitle").click();
  ...
  for(let i=0;i<5;i++)await page.locator("#gpsTitle").click();
  if(await page.evaluate(()=>window.eval("droggerOwnerModeActive")))throw new Error("five taps did not close hidden mode");
  console.log("OK: Drogger hidden UI, registration, metadata, grouped deletion and undo");
