...
- `validate-mobile-landscape.js` — 縦固定メタ・マニフェスト・JavaScriptロックの解除、844×390横画面で遮断画面が出ないこと、初回画面の全操作へスクロールできること、PC専用の写真位置が非表示であること、縦画面は左端0、横画面は図面・操作キャンバス左端が「細」の左端へ一致することを確認。
