...
- コミット: `44f1777`
- ブランチ: `main`
- 内容: 2026-08-22時点のローカル検証版。法務局データの容量先読みと、GPS非依存のスマホ方位追従、iPhoneのファイル選択時許可先取り、方位ボタンの直接ON/OFF、応答高速化を含む。公開は別工程。
...
- 確認内容:
  ...
  - iPhoneはファイル選択のchangeイベント中に方位センサー許可を先取りし、読込完了後に追加操作なしで開始する。サンプル図面はサンプルボタン操作中に先取りする。
  - 方位ボタンは設定ポップアップを開かず、押すたびに自動回転を直接ON/OFFする。
  - Android/WebViewで `absolute=false` の方位イベントしか届かない場合も、有限なalpha値を回転入力として使用する。
  - 方位変化は最短回転方向へ指数補間する。応答時定数45ms・目標追従率72%とし、通常図面は最大60fps、大規模図面は既存の約32ms描画制限を使う。
  - ホームを押すと追従、補間フレーム、再開待ちを停止し、図面の読み込み時の角度へ戻す。
- 検証: `validate-compass-follow.js`（iPhone相当の権限APIモックを含め3回連続成功）、`validate-performance-indexes.js`、`validate-gps-startup-mode.js`、`validate-real-sfc-rendering.js sample.sfc`。実機の許可ダイアログ経路は次回端末確認で再評価する。
