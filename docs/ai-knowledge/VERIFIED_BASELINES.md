...
- コミット: `bdc4975`
- ブランチ: `main`
- 内容: 2026-08-22時点の検証版。初回画面からの前回作業復元、法務局データの容量先読み、図面座標系未設定時の誤距離防止、主図面部分を使う距離判定、現在地座標系での保存済み別地点ラスター抑止、GPS中の航空写真固定、GPS非依存・方位ボタン手動開始のスマホ方位追従、追従中の明瞭な青表示、直接ON/OFF、応答高速化、AGPL v3の公開ライセンスとアプリ内法的表示を含む。
...
- 確認内容:
  ...
  - 図面の座標系が未設定の場合、端末現在地の座標系や自動記憶値を図面の座標系と仮定しない。誤った数値距離を出さず、未設定通知後に現在地座標系と航空写真へ切り替える。
  - スマホで最初にGPSを取得した場所の座標系は端末既定として保存するが、設定補助に限定し、図面との距離計算へ自動適用しない。正確な距離には図面別の手動座標系を使う。
...
- 検証: `validate-gps-startup-mode.js`（端末既定を距離へ使わないこと、図面別手動指定、座標系未設定通知、主図面範囲、固定メッシュ変換、保存済み別地点ラスター抑止、現在地の最新航空写真固定、GPS終了時の復元を含む）、`validate-performance-indexes.js`、`validate-compass-follow.js`、`validate-default-settings.js`、`validate-last-work-recovery.js`、`validate-real-sfc-rendering.js sample.sfc`。
