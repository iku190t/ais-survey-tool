...
- コミット: `7179fe4`
- ブランチ: `main`
- 内容: 2026-08-23時点の検証版。統一した起動画面ボタン、初回画面からの前回作業復元、法務局データの容量先読み、スマホ現在地による図面別座標系の初回設定、主図面部分を使う距離判定、現在地座標系での保存済み別地点ラスター抑止、GPS中の航空写真固定、GPS非依存・方位ボタン手動開始のスマホ方位追従、追従中の明瞭な青表示、直接ON/OFF、応答高速化、AGPL v3の公開ライセンスとアプリ内法的表示を含む。
