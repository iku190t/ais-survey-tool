...
(async()=>{
  ...
  if(!two.sheet.includes('<mergeCell ref="A28:M28"/>')||!two.sheet.includes('<mergeCell ref="A29:M29"/>')||!two.sheet.includes('<mergeCell ref="A30:M30"/>'))throw new Error("2枚形式のコメントが写真の下へ配置されていません");
  const twoPhotoAnchor=drawingAnchorForName(two.drawing,"写真 1");
  if(!twoPhotoAnchor.includes("<xdr:col>0</xdr:col>")||!/<xdr:to>[\s\S]*?<xdr:col>12<\/xdr:col>/.test(twoPhotoAnchor))throw new Error("2枚形式の写真が横幅いっぱいではありません");
  ...
  if(three.sheet.includes('<c r="H20"')||three.sheet.includes('<c r="H40"'))throw new Error("3枚形式の次コメント直上に罫線が残っています");
  if(four.sheet.includes('<c r="H15"')||four.sheet.includes('<c r="H30"')||four.sheet.includes('<c r="H45"'))throw new Error("4枚形式の次コメント直上に罫線が残っています");
  ...
  console.log(`photo album runtime checks passed (2-photo media=${two.media}, 3-photo media=${three.media}, 4-photo media=${four.media}, 6-photo media=${six.media}, spread media=${spread.media})`);
